...
def remove_inflight_events(db,events,alert_params):
    ...
    for event in events:
        #todo, add and alert_name = x
        alerted = db['inflight_alerts'].count_documents({'slots.events':{ '$elemMatch': {'eventid':event['eventid']}}})
        if not alerted:
            new_events.append(event)
    return new_events
...
def save_alert(db, new_alert):
    ...
    new_alert['utcepoch'] = calendar.timegm(toUTC(new_alert['utctimestamp']).utctimetuple())
...
def get_athena_query(criteria,config):
    ...
    '''
    hour,month,day,year,last_hour_hour, last_hour_month,last_hour_day, last_hour_year=get_date_parts()
    ...
    *
    ...
    limit 1000
    """.format(database,table,criteria,year,month,day,hour,last_hour_year,last_hour_month,last_hour_day,last_hour_hour)
    return query
...
def get_sequence_alert_shell(alert_params):
    ...
    alert["expiration"]=alert_params.get("expiration",(toUTC(alert["utctimestamp"]) + offset).isoformat())
...
def get_athena_events(criteria,config,athena,session):
    ...
    athena_response=run_query(athena,athena_query,config.athenadatabase,default_bucket(session))
    while query_status == 'QUEUED' or query_status == 'RUNNING' or query_status is None:
        query_status = athena.get_query_execution(QueryExecutionId=athena_response['QueryExecutionId'])['QueryExecution']['Status']['State']
...
def main(config):
    ...
    client = pymongo.MongoClient('mongodb://{}:{}/'.format(config.mongohost,config.mongoport))
