...
class TestAlertFunctions(object):
    ...
    def test_expire_sequence_alerts(self,mongo_connection):
        ...
        alert_shell={
            "utctimestamp": last_week.isoformat(),
            "lifespan": "1 day"}
    ...
    def test_save_resolved_threshold_alert(self,mongo_connection):
        ...
        alert_shell["summary"] = "{{events.0.eventname}} by {{events.0.useridentity.type}} {{metadata.count}} mfa:{{events.0.additionaleventdata.mfaused}}"
        alert_shell["event_snippet"]="{{eventname}}/{{responseelements.consolelogin}} mfa:{{additionaleventdata.mfaused}} from {{sourceipaddress}}"
    ...
    def test_save_resolved_sequence_alert(self,mongo_connection):
        ...
        alert_shell["summary"] = "{{slots.0.events.0.eventname}} by {{slots.0.events.0.useridentity.type}} {{metadata.count}} mfa:{{slots.0.events.0.additionaleventdata.mfaused}}"

        alert_shell["slots"]=[]
        # make a slot of threshold alert + events that trigger it
        alert_slot=get_threshold_alert_shell({})
        alert_slot["event_snippet"]="{{eventname}}/{{responseelements.consolelogin}} mfa:{{additionaleventdata.mfaused}} from {{sourceipaddress}}"
